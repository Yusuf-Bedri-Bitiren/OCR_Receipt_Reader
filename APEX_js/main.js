...
async function runOcrVariants(imagePath, lang = 'tur', test = false) {
  const psmList = [11, 4, 6, 3];
  const results = [];

  for (const psm of psmList) {
    const text = await extractText(imagePath, psm, lang);
    if (test) {
      console.log(`\n[Raw Text PSM ${psm}]:\n`, text);  // Print out raw text for debugging
    }
    results.push(extractFields(text));
  }

  const final = mergeFieldResults(results);
  if (test) console.log("\n[Final Merged Fields]:", final);
  return final;
}
...
// Run it
(async () => {
  const imagePath = 'C:/Users/LENOVO/Desktop/Python/Karel/receipts/S35.jpg';
  await runOcrVariants(imagePath, 'tur', true);
})();